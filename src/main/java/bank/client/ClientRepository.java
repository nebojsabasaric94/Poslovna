...
public interface ClientRepository extends CrudRepository<Client, Long>{
	...
	@Query("select c from Client c where c.jmbg like CONCAT(:jmbg,'%') and c.firstName like CONCAT(:firstName,'%') and c.lastName like CONCAT(:lastName,'%')"
			...
			+ " c.email like CONCAT(:email,'%') and c.phone like CONCAT(:phone,'%') and c.addressForStatements like CONCAT(:addressForStatements,'%') and c.emailStatements = :emailStatements")

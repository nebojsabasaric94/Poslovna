...
public interface CurrencyRepository extends PagingAndSortingRepository<Currency, Long> {
	...
	@Query("select c from Currency c where c.name like CONCAT(:name,'%') and CAST(c.country.id AS string) like :country_id and c.domicilna = :domicilna and c.official_code like CONCAT(:official_code,'%')")

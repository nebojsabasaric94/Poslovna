...
public interface InterbankTransferRepository extends PagingAndSortingRepository<InterbankTransfer, Long> {
	...
	@Query("select i from InterbankTransfer i where i.date like CONCAT(:date,'%') and CAST(i.bank.id AS string) like :bank and CAST(i.senderBank.id AS string) like :senderBank and i.typeOfMessage like 'MT102'")
